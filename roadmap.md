...
JPEG 2000에서 사용하는 기본 타입 별칭과 상수를 정의한다.

```
- OPJ_INT32, OPJ_UINT32, OPJ_INT16, OPJ_UINT16, OPJ_BYTE 등 타입 별칭
- OPJ_J2K_MAXRLVLS (33), OPJ_J2K_MAXBANDS (97)
...
Big-endian/Little-endian 바이트 읽기/쓰기, 추상 스트림 trait을 정의한다.

```
- read_u8, read_u16_be, read_u32_be, read_u64_be
- write_u8, write_u16_be, write_u32_be
- Stream trait (read, write, seek, skip, tell, remaining)
- SliceReader, VecWriter 구현체
```

**테스트**: `tests/phase0_stream.rs`
```rust
#[test] fn read_u16_be()              // [0xFF, 0x4F] → 0xFF4F
#[test] fn read_u32_be()              // 4바이트 BE 읽기
...
#[test] fn vec_writer_flush()         // VecWriter → Vec<u8> 변환
...
### Step 2.1 — MQ 코더 상태 테이블 (`mqc_states.rs`)
...
**테스트**: `tests/phase2_mqc_states.rs`
```rust
...
### Step 2.2 — MQ 인코더 (`mqc_enc.rs`)

```
- MqcEncoder: init, encode(ctx, bit), bypass_enc, flush
- 19개 컨텍스트 (MQC_NUMCTXS)
- resetstates, setstate
- bypass(raw) 모드, erterm, segmark, restart
```

**테스트**: `tests/phase2_mqc_enc.rs`
```rust
...
#[test] fn resetstates()              // 상태 리셋 후 기본값
#[test] fn bypass_mode_encode()       // raw 바이패스 모드
#[test] fn erterm_flush()             // 에러 종료 모드
#[test] fn segmark_encode()           // 세그먼트 마커
```

### Step 2.3 — MQ 디코더 (`mqc_dec.rs`)

```
- MqcDecoder: init, raw_init, decode(ctx), finish
- renormalize, bytein (0xFF 핸들링)
```

**테스트**: `tests/phase2_mqc_dec.rs`
```rust
